      ...
      if (this.state.name.length < 5) {
        ...
        .then((snapshot) => {
          if (snapshot.val()) {
            ...
            firebaseApp.database().ref('usernameList').child(this.props.appStore.username.toLowerCase()).remove()
            .then(() => {
              firebaseApp.database().ref('users').child(this.props.appStore.uid).update({username:this.state.name})
              firebaseApp.database().ref('usernameList').child(this.state.name.toLowerCase()).set(this.props.appStore.uid)
